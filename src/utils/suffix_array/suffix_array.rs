...
fn suffix_array_naive(data: Vec<u32>) -> Vec<usize> {
    let length = data.len();

    let mut suffix_idx = (0..data.len()).collect::<Vec<_>>();
    suffix_idx.sort_by(|&(mut left): &usize, &(mut right): &usize| {
        if left == right {
...
fn suffix_array_doubling(data: Vec<u32>) -> Vec<usize> {
    let length = data.len();

    let mut suffix_idx = (0..data.len()).collect::<Vec<_>>();
    let mut rank = data;

    let mut next_rank = vec![0; rank.len()];
    ...
    while prefix_len < length {
        let compare_suffix = |&left: &usize, &right: &usize| {
            ...
            match (left + prefix_len < length, right + prefix_len < length) {
                ...
                (true, true) => rank[left + prefix_len].cmp(&rank[right + prefix_len]),
            }
        };
...
fn suffix_array_induced_sorting(data: Vec<u32>, alphabet_max: u32) -> Vec<usize> {
    let length = data.len();
    let mut suffix_idx = vec![0; length];
    ...
    let mut bucket_l_start = vec![0; alphabet_max as usize + 1];
    let mut bucket_s_start = vec![0; alphabet_max as usize + 1];
    for (&is_s_type, &data) in iter::zip(&is_s_type, &data) {
        if is_s_type {
            bucket_l_start[data as usize + 1] += 1;
        } else {
            bucket_s_start[data as usize] += 1;
        }
    }
    ...
    let mut bucket_cursor = vec![0; alphabet_max as usize + 1];
    ...
    let induced_sort =
        |suffix_idx: &mut [usize], bucket_cursor: &mut [usize], lms_positions: &[usize]| {
            ...
            for i in 0..length {
                ...
                if sa_value > 1 && !is_s_type[sa_value - 2] {
    ...
    let mut lms_index = vec![0; length + 1];
    let mut num_lms = 0;
    for i in 1..length {
    ...
    if num_lms > 0 {
        let mut sorted_lms_positions = suffix_idx
            ...
            .collect::<Vec<_>>();
        let mut reduced_data = vec![0; num_lms];
        let mut reduced_alphabet_max = 0;
        reduced_data[lms_index[sorted_lms_positions[0]] - 1] = 0;
        for i in 1..num_lms {
        ...
        for (i, reduced_sa_value) in reduced_suffix_array.into_iter().enumerate() {
            sorted_lms_positions[i] = lms_positions[reduced_sa_value];
        }
        drop(lms_positions);
...
fn suffix_array_inner(data: Vec<u32>, alphabet_max: u32) -> Vec<usize> {
    match data.len() {
        0..=1 => (0..data.len()).collect(),
        2 => {
            if data[0] < data[1] {
                vec![0, 1]
            } else {
...
pub(crate) fn suffix_array(data: &[u32]) -> Vec<usize> {
    ...
    suffix_array_inner(data.to_vec(), alphabet_max)
}
...
mod tests {
    ...
    fn verify_all(array: &[u32], expected_idx: &[usize]) {
        ...
        let suffix_idx_optional = suffix_array(array);
