...
fn suffix_array_induced_sorting(data: Vec<usize>, alphabet_max: usize) -> Vec<usize> {
    ...
    let induced_sort = |suffix_idx: &mut Vec<usize>, lms_positions: Vec<usize>| {
        ...
        for i in (0..length).rev() {
            let sa_value = suffix_idx[i];
            if sa_value >= 2 && is_s_type[sa_value - 2] {
                bucket_cursor[data[sa_value - 2] + 1] -= 1;
                let pos = bucket_cursor[data[sa_value - 2] + 1];
