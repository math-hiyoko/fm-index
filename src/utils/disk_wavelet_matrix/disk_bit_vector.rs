...
impl DiskBitVector {
    ...
    pub(super) fn try_clone(&self) -> PyResult<Self> {
        ...
        blocks_file
            .set_len(self.blocks_mmap.len() as u64)
            .map_err(PyOSError::new_err)?;
        #[allow(unsafe_code)]
        let mut blocks_mmap =
            unsafe { MmapMut::map_mut(&blocks_file).map_err(PyOSError::new_err)? };
        blocks_mmap.copy_from_slice(&self.blocks_mmap[..]);

        let select_index_file = [
            {
                let file = tempfile().map_err(PyOSError::new_err)?;
                file.set_len(self.select_index_mmap[0].len() as u64)
                    .map_err(PyOSError::new_err)?;
                file
            },
            {
                let file = tempfile().map_err(PyOSError::new_err)?;
                file.set_len(self.select_index_mmap[1].len() as u64)
                    .map_err(PyOSError::new_err)?;
                file
            },
        ];
        let mut select_index_mmap = [
            #[allow(unsafe_code)]
            unsafe {
        ...
        select_index_mmap[0].copy_from_slice(&self.select_index_mmap[0][..]);
        select_index_mmap[1].copy_from_slice(&self.select_index_mmap[1][..]);

        let [select_index_mmap_0, select_index_mmap_1] = select_index_mmap;
...
impl BitVectorTrait for DiskBitVector {
    ...
    fn values(&self) -> PyResult<Vec<BlockType>> {
        Ok(cast_slice(&self.blocks_mmap[..]).to_vec())
    }
    ...
    fn select(&self, bit: bool, mut kth: usize) -> PyResult<Option<usize>> {
        ...
        let select_index_slice: [&[usize]; 2] = [
            cast_slice(&self.select_index_mmap[0][..]),
            cast_slice(&self.select_index_mmap[1][..]),
        ];
        let ranks_slice: &[usize] = cast_slice(&self.ranks_mmap[..]);
        let blocks_slice: &[BlockType] = cast_slice(&self.blocks_mmap[..]);
        ...
        let block_index = {
            ...
            debug_assert!(right <= self.blocks_mmap.len());
            while left + 1 < right {
